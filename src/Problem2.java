...
public class Problem2 {
    private static class Set{
        ...
        private int red;
        private int green;
        private static Pattern pBlue = Pattern.compile("(\\d+) blue"); // (\d+) -> Captures any number
        private static Pattern pRed = Pattern.compile("(\\d+) red");
        private static Pattern pGreen = Pattern.compile("(\\d+) green");
        public Set(){
            blue=0;
            red=0;
            green=0;
        ...
        public boolean getAmounts(String set){
            Matcher matcherBlue = pBlue.matcher(set);
            Matcher matcherRed = pRed.matcher(set);
            Matcher matcherGreen = pGreen.matcher(set);
